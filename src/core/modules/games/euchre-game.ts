...
export class Euchre extends BaseGame {
    ...
    public async startRound(): Promise<void> {
        ...
        const promptReplace = async (index: number): Promise<void> => {
            ...
            collector.once('collect', interaction => {
                interaction.update({ embeds: [ generateEmbed('success', { title: 'Success!' }) ], components: [], files: [] })
                this.players[3].hand.splice(Number.parseInt(interaction.values[0]), 1, top)
                this.trump = top.suit
                promptThree(index)
            })
        }
    ...
    private async round(leader: number, solo = false, table: string[] = [], lead?: 'H' | 'D' | 'S' | 'C', index = 0): Promise<void> {
    ...
    private async score(leader: number, solo: boolean): Promise<void> {
        const winningTeam = this.team1.tricks > this.team2.tricks ? this.team1 : this.team2
