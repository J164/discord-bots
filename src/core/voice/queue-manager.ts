...
export class QueueManager {
    ...
    public async addToQueue(items: QueueItem[], position: number): Promise<void> {
        ...
        if (position === 0) {
            this.queue = [ ...this.queue.reverse(), ...items.reverse() ].reverse()
            this.queueLock = false
            return
        }
        const queueEnd = this.queue.splice(position)
        this.queue = [ ...this.queue, ...items, ...queueEnd ]
        this.queueLock = false
    }

    public async connect(voiceChannel: VoiceChannel): Promise<boolean> {
        this.transitioning = true
        if (!await this.voiceManager.connect(voiceChannel) || this.queue.length === 0) {
            this.reset()
            return false
        }
    ...
    public loopQueue(): InteractionReplyOptions {
        if (!this.voiceManager.isActive() || this.queue.length === 0) {
    ...
    public async shuffleQueue(): Promise<boolean> {
        ...
        for (let index = this.queue.length - 1; index > 0; index--) {
            ...
            const temporary = this.queue[index]
            this.queue[index] = this.queue[randomIndex]
            this.queue[randomIndex] = temporary
        }
        this.queueLock = false
        return true
    }
    ...
    public async getQueue(): Promise<QueueItem[][]> {
        ...
        for (let r = 0; r < Math.ceil(this.queue.length / 25); r++) {
            ...
            for (let index = -1; index < 25; index++) {
                if (r * 25 + index > this.queue.length - 1) {
                    break
                }
                if (r === 0 && index === -1) {
                ...
                queueArray[r].push(this.queue[r * 25 + index])
            }
        }
        this.queueLock = false
        return queueArray
