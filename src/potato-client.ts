...
export class PotatoClient extends Client {
  ...
  public constructor(clientOptions: ClientOptions) {
    ...
    this.once('ready', async () => {
      ...
      const gradeTask = cron.schedule(env.GRADE_UPDATE_INTERVAL, async () => {
        for (const user of Object.keys(this._privateData.ircAuth)) {
          const report = await gradeReport(this._privateData.ircAuth[user], this._databaseManager, gradeTask);
          if (!report) return;
