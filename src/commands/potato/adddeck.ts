...
async function addDeck(info: GlobalChatCommandInfo): Promise<InteractionReplyOptions> {
    if (info.interaction.user.id !== process.env.ADMIN && info.interaction.user.id !== process.env.SWEAR && info.interaction.user.id !== process.env.MAGIC) {
    ...
    if (decks.some(a => a.url === info.interaction.options.getString('url'))) {
    ...
    let name: string
    try {
        const fields = info.interaction.options.getString('url').split('/')
    ...
    await info.database.insert('mtg_decks', { url: info.interaction.options.getString('url') })
