...
class Euchre {
    ...
    async startRound() {
        ...
        await this.sendCards(`C:/Users/jacob/Downloads/Bot Resources/img_files/cards/${this.gameState['top']['code']}.png`, 'Top of Stack:')
        let playerUsers = []
        for (const player of this.players) {
            playerUsers.push(player)
        }
        for (const player of this.players) {
            const response = await this.askPlayer(player['player'], `Would you like to pass or have ${this.players[3]['player'].username} pick it up?`, ['Pick it up', 'Pass'])
            if (response == 0) {
                this.gameState['trump'] = this.gameState['top']['suit']
                this.players[3]['hand'][await this.askPlayer(this.players[3]['player'], 'What card would you like to replace?', this.getCardNames(this.players[3]['hand']))] = this.gameState['top']
                this.sendHand(this.players[3])
                if (await this.askPlayer(player['player'], 'Would you like to go alone?', ['Yes', 'No']) == 0) {
                    switch (player['id']) {
                        case 0:
        ...
        for (let [i, player] of this.players.entries()) {
            ...
            const response = await this.askPlayer(player['player'], 'What would you like to be trump?', availableSuits)
            if (response != 3) {
    ...
    async tricks(activePlayers, leader, solo) {
        for (let r = 0; r < 5; r++) {
            ...
            for (const player of activePlayers) {
                ...
                const response = await this.askPlayer(player['player'], 'What would you like to play?', this.getCardNames(availableHand))
